function Section = CalculateShearResistance(Section,panel,location)

% Define Variables
dw = Section.dw;
tw = Section.tw;
tf_top  = Section.tf_top;
bf_top  = Section.bf_top;
tf_bottom = Section.tf_bottom;
bf_bottom = Section.bf_bottom;
Es = Section.Es;
Fy = Section.Fy;
Lb = Section.Lb;

% Determine compression/tension flange
if strcmp(lower(location),'positive')
...
elseif strcmp(lower(location),'negative')
...
if dw/tw > 1.40*sqrt((Es*k)/Fy)
    C = (1.57/(dw/tw)^2)*((Es*k)/Fy); % (6.10.9.3.2-6)
elseif dw/tw > 1.12*sqrt((Es*k)/Fy)
    C = (1.12/(dw/tw))*((Es*k)/Fy); % (6.10.9.3.2-5)
else
    C = 1; % (6.10.9.3.2-4)
end
