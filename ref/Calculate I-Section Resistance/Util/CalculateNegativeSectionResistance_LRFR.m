function s = CalculateNegativeSectionResistance_LRFR(s)
...
tf_bottom = s.tf_bottom(2); % Thickness of bottom flange (compression flange)
bf_bottom = s.bf_bottom(2); % Width of bottom flange (tension flange)
...
lf = bf_bottom/(2*tf_bottom); % slenderness ratio for the compression flange
...
% (conservative)
s.Fn_Strength1Neg = min(Fnc_neg_LB, Fnc_neg_LT);
s.Fn_Service2Neg = 0.8*Fy;
