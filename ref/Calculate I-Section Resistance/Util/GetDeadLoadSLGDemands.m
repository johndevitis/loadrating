function d = GetDeadLoadSLGDemands(s,d)

% Define Variables used in GetFEApproximation -----------------------------

Spans = s.nSpans;
for ii = 1:Spans
    SpanLength(ii) = round(s.spanLength/12)'; %[ft]
end
TotalLength = s.spanLength/12*Spans; %[ft]
Es = s.Es; %[psi]
...
[Delta_Min, Delta_Max, D] = GetDisplacementVector(numDOF, condK, condInd, Fixed);
...
[M_Max, M_Min, V_Max, V_Min, ~, ~] = GetMemberForceVector(D, Es, numEle);
